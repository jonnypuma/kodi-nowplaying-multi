...
def pick_thumb_filename(downloaded_art, media_type=None):
    """Return preferred overview tile image filename, or None."""
    picked = pick_thumb_art(downloaded_art, media_type=media_type)
    return picked[0] if picked else None



def pick_thumb_art(downloaded_art, media_type=None):
    """Return (filename, art_key) for overview tile primary art.

    Movies and TV shows share one order: landscape, then fanart with the banner
    overlaid, then fanart alone, then the show thumb. Episode screenshots are
    not stored under ``thumb`` (see art download), so that last step is the
    show composite only.
    """
    if not isinstance(downloaded_art, dict):
        return None
    if media_type in ("movie", "episode"):
        landscape = downloaded_art.get("landscape")
        if landscape:
            return landscape, "landscape"
        fanart = pick_fanart_filename(downloaded_art)
        banner = downloaded_art.get("banner")
        if banner and fanart:
            return banner, "banner"
        if fanart:
            return fanart, "fanart"
        show_thumb = downloaded_art.get("thumb")
        if show_thumb:
            return show_thumb, "thumb"
        if banner:
            return banner, "banner"
        for key in _c.THUMB_ART_PRIORITY:
            if key in ("landscape", "banner", "thumb", "fanart"):
                continue
