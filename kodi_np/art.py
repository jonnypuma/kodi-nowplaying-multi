...
def _prepare_and_download_art_locked(item, session_id, server, progress_cb=None, share_context=None):
    ...
    # Episode item.thumb is a screenshot. The overview tile wants the show's
    # composite thumb (fanart + title already baked in), not that still.
    if media_type == "episode" and not (buckets.get("tvshow") or {}).get("thumb"):
        art_map.pop("thumb", None)
                            ...
                            if isinstance(file_info, dict):
                                ...
                                if file_type == "file" and file_path:
                                    ...
                                    stem_map = {
                                        ...
                                        "thumb": ("thumb.png", "thumb.jpg", "thumb.jpeg", "thumb.webp"),
                                    ...
                                    for art_key, names in stem_map.items():
                                        if art_key == "thumb" and media_type != "episode":
                                            continue
                                        if base_name in names and not art_map.get(art_key):
                                            art_map[art_key] = file_path
                                            if enable_share and media_type == "episode" and art_key == "thumb":
                                                key_scope[art_key] = "tvshow"
                                                buckets.setdefault("tvshow", {})[art_key] = file_path
